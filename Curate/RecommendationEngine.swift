...
//  =====================================
//  This Thompson Sampling-based recommendation engine has been replaced by
//  the HybridRecommender in CandidatePool/HybridRecommender.swift.
//
//  The hybrid candidate pool architecture provides:
//  - Better global caching (shared pools reduce API calls ~90%)
//  - More deterministic, explainable recommendations
//  - Per-user overlays for personalization
//  - LLM-only for intent parsing, not per-track decisions
//
//  To re-enable: Remove the comment markers below and update the station
//  service to use RecommendationEngine instead of HybridRecommender.
//
...
// NOTE: FeedbackType enum is now defined in Feedback.swift
// It was moved there to avoid duplication.
